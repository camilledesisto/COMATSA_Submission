...
density_heatmap <- ggplot(data = lemur_densities2, 
                          aes(x = Species, y = SiteTransect, fill = Density_km2)) +
  ...
  scale_fill_viridis_c(
    ...
    option = "magma",
    na.value = "grey80",
    name = expression("Population density (Ind./" * km^2 * ")")
  ) +
  # Add a dummy tile for the 'No data' legend key
  geom_tile(aes(x = Inf, y = Inf, fill = NA, color = "No data"),
            width = 0.5, height = 0.5, show.legend = TRUE) +
  scale_color_manual(
    name = NULL,
    values = c("No data" = "grey80"),
    guide = guide_legend(
      override.aes = list(
  ...
  guides(
    ...
    color = guide_legend(
      title = NULL,
      override.aes = list(shape = 22, size = 6, fill = "grey80"),
...
lemur_densities3 <- lemur_densities2 %>%
  dplyr::select(SiteTransect, Species, Density_km2) %>%
  distinct() %>%
  tidyr::pivot_wider(names_from = Species, values_from = Density_km2) %>%
  dplyr::mutate(across(where(is.numeric), ~ round(., 2)))

write.csv(lemur_densities3, "lemur_densities3.csv")
...
density_summary <- lemur_densities2 %>%
  group_by(Species) %>%
  summarise(
    mean = mean(Density_km2, na.rm = TRUE),
    n = sum(!is.na(Density_km2)),
    se = sd(Density_km2, na.rm = TRUE) / sqrt(n),
    lower95 = mean - 1.96 * se,
    upper95 = mean + 1.96 * se
  ) %>%
  dplyr::select(Species, mean, lower95, upper95)
...
```{r Variable Importance Figure - code outdated for non-scaled variables}
