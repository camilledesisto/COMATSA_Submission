...
albi_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/albi_dat.csv")
allo_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/allocebus_dat.csv")
avahi_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/avahi_dat.csv")
cheiro_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/cheirogaleus_dat.csv")
lepi_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/lepilemur_dat.csv")
rubri_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/rubriventer_dat.csv")
prop_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/prop_dat.csv")
micro_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/microcebus_dat.csv")
hapa_density <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/hapa_dat.csv")
...
habitat <- read.csv("~/Documents/GitHub/COMATSA_Submission/Data/habitat_covs.csv")
...
lemur_densities3 <- lemur_densities2 %>% dplyr::select(SiteTransect, Density_km2, Species)%>% mutate(Density_km2 = round(Density_km2,0)) %>%
  pivot_wider(names_from = Species, values_from = Density_km2) 

write.csv(lemur_densities3, "lemur_densities_transect.csv")
...
DEM <- raster("~/Downloads/SpatialFiles/ASTGTMV003_S15E049_dem.tif")
FH <- raster("~/Documents/GitHub/COMATSA_GitHub2/Data/covariate_tif/Forest_height_2019_SAFR.tif")
TRI <- raster("~/Documents/GitHub/COMATSA_GitHub2/Data/covariate_tif/TRI_Nov2023.tif")
FL <- raster("~/Downloads/Comatsa_ForestLoss/FARForestLoss20102021.tif")
...
FPred <- raster("~/Downloads/clipped_deforestation_2050.img")
...
```{r Create Forest Loss DFs for Each Lemur Species}
...
writeRaster(
  stacked_habitat$FLP_prop,
  filename  = "~/Documents/GitHub/COMATSA_Submission/Data/FLP_prop.tif",
  format    = "GTiff",
  overwrite = TRUE
)
...
rubri_dredge <- dredge(rubri_scale)
...
cheiro_dredge <- dredge(cheiro_scale)
...
micro_full <- lmer(data = densities_habitat_micro[!is.na(densities_habitat_micro$Density_km2),] , sqrt(Density_km2) ~ Elevation + CanopyHeight + Ruggedness +ForestLossProp+ (1|Site),na.action = "na.fail")
micro_scale <- scale_mod(micro_full)

micro_dredge <- dredge(micro_scale)
...
##loop not loop
coef_Intercept_micro<-  rnorm(1000, mean=7.5009 , sd=1.1451)
coef_FLP_micro<-  rnorm(1000, mean=-0.4432 , sd=0.7576  )
coef_CH_micro<-  rnorm(1000, mean=-0.4691, sd=0.8056)
coef_Rug_micro<-  rnorm(1000, mean=-0.1498, sd=0.6319 )
coef_Elev_micro<-  rnorm(1000, mean=0.1614  , sd=0.5836)
...
avahi_dredge <- dredge(avahi_scale)
...
summary_allo_coef <- data.frame(summary_allo_model_avg$coefmat.full)
summary_avahi_coef <-data.frame(summary_avahi_model_avg$coefmat.full)
summary_albi_coef <-data.frame(summary_albi_model_avg$coefmat.full)
summary_cheiro_coef <-data.frame(summary_cheiro_model_avg$coefmat.full)
summary_hapa_coef <-data.frame(summary_hapa_model_avg$coefmat.full)
summary_lepi_coef <-data.frame(summary_lepi_model_avg$coefmat.full)
summary_micro_coef <-data.frame(summary_micro_model_avg$coefmat.full)
summary_rubri_coef <-data.frame(summary_rubri_model_avg$coefmat.full)
summary_prop_coef <-data.frame(summary_prop_model_avg$coefmat.full)

summary_allo_coef$Species <- "A. trichotis"
summary_avahi_coef$Species <- "A. laniger"
summary_albi_coef$Species <- "E. albifrons"
summary_cheiro_coef$Species <- "C. crossleyi"
summary_hapa_coef$Species <- "H. occidentalis"
summary_lepi_coef$Species <- "L. seali"
summary_micro_coef$Species <- "M. lehilahytsara"
summary_rubri_coef$Species <- "E. rubriventer"
summary_prop_coef$Species <- "P. candidus"

summary_allo_coef$Variable <- rownames(summary_allo_coef)
summary_avahi_coef$Variable <- rownames(summary_avahi_coef)
summary_albi_coef$Variable <- rownames(summary_albi_coef)
summary_cheiro_coef$Variable <- rownames(summary_cheiro_coef)
summary_hapa_coef$Variable <- rownames(summary_hapa_coef)
summary_lepi_coef$Variable <- rownames(summary_lepi_coef)
summary_micro_coef$Variable <- rownames(summary_micro_coef)
summary_rubri_coef$Variable <- rownames(summary_rubri_coef)
summary_prop_coef$Variable <- rownames(summary_prop_coef)

summary_coef_total <- rbind(summary_allo_coef, summary_avahi_coef, summary_cheiro_coef, summary_albi_coef, summary_rubri_coef, summary_hapa_coef, summary_lepi_coef, summary_micro_coef, summary_prop_coef) 

colnames(summary_coef_total) <- c("Estimate", "Std. Error", "Adjusted SE", "Z values", "P-value", "Species", "Variable")

summary_coef_total <- summary_coef_total[,c(6,7,1,2,5)]
summary_coef_total$Estimate <- round(summary_coef_total$Estimate,3)
summary_coef_total$`Std. Error`<- round(summary_coef_total$`Std. Error`,3)
summary_coef_total$`P-value`<- round(summary_coef_total$`P-value`,3)

write.csv(summary_coef_total, "summary_coef_total_comatsa.csv")


importance_df <- data.frame(rbind(importance_allo_model_avg, importance_avahi_model_avg, importance_cheiro_model_avg, importance_albi_model_avg, importance_rubri_model_avg, importance_hapa_model_avg, importance_lepi_model_avg, importance_micro_model_avg, importance_prop_model_avg))
