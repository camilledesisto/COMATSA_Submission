...
ggsave(
  filename = "pop_distribution_pa_2050.png",
...
ggsave(
  filename = "pop_distribution_buffer_2020.png",
...
ggsave(
  filename = "pop_distribution_buffer_2050.png",
...
ggsave(
  filename = "plot_density_predict.png",
...
Canopy_Height_plot <- ggplot() +
  geom_raster(data = fh_df, aes(x = x, y = y, fill = canopy_height)) +
  geom_sf(data = bound, fill = NA, color = "black", linewidth = 1.5) +
  coord_sf(expand = FALSE) +
  scale_fill_viridis_c(option = "magma", name = "Canopy\nHeight (m)", direction = -1) +
  theme_void() +
  theme(
    legend.position = "right",
    axis.title = element_blank()
...
Forest_overlay_plot <- ggplot() +
  ...
  geom_sf(
    data = roads,
    aes(color = "Main Roads"),
    linewidth = 2
  ) +
  scale_color_manual(
    values = c("Main Roads" = "brown"),
    name = NULL
  ) +
  geom_sf(
    data = points_sf,
