...
transect_lengths <- read.csv("D:/GitHub/COMATSA_GitHub2/Data/Transect_Length.csv")

transect_lengths$Site_Transect <- paste(transect_lengths$Site,"_",as.numeric(transect_lengths$Transect), sep="")

survey_all <- read.csv("D:/GitHub/COMATSA_GitHub2/Data/lemurs_primary_in_transect_april30.csv")
...
```{r tristan seasonality tests in progress}
...
ggplot(test_micro, aes(x = calendar, y = EncounterRate)) +
  ...
  labs(
    title = "Cheiro Encounter Rate Over Time",
    x = "Calendar Date",
    y = "Encounter Rate",
    color = NULL
  ) +
...
ggplot(test_micro, aes(x = calendar_num, y = EncounterRate)) +
  ...
  labs(
    title = "Microcebus Encounter Rate fit Sine wave",
    x = "Calendar",
    y = "Encounter Rate"
  ) +
  theme_minimal()
...
write.csv(allocebus_dat,"~/COMATSA_Submission/Data/allocebus_dat.csv")
...
gof_ds(model10_microcebus)#significant
gof_ds(model11_microcebus)#significant

#Compare AIC calues
AIC(model1_microcebus, model2_microcebus, model3_microcebus, model4_microcebus, model5_microcebus, model6_microcebus, model7_microcebus, model8_microcebus, model9_microcebus, model10_microcebus, model11_microcebus)
...
write.csv(microcebus_dat,"~/COMATSA_Submission/Data/microcebus_dat.csv")
...
write.csv(cheirogaleus_dat,"~/COMATSA_Submission/Data/cheirogaleus_dat.csv")
